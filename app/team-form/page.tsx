  ...
  return (
    <main className="min-h-screen stadium-page px-4 pb-28 pt-4 text-white md:px-6">
      <div className="mx-auto max-w-5xl space-y-6">
        <section className="glass-card glow overflow-hidden rounded-[2rem] p-6">
          <Link href="/" className="text-sm font-bold text-cyan-300">
            ← Zurück zu den Picks
          </Link>

          <div className="mt-7 flex items-center gap-5">
            <TeamBadge team={team || "Team"} size={88} />
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-300">
                Letzte 10 Spiele mit Ergebnissen und Gegnern.
              </p>
            </div>
          </div>

          <div className="mt-7 grid grid-cols-4 gap-3">
            <Stat label="Spiele" value={String(games.length)} />
            <Stat label="Siege" value={String(wins)} />
            <Stat label="Remis" value={String(draws)} />
            <Stat label="Niederl." value={String(losses)} />
          </div>
        </section>
        ...
        <section className="grid gap-4">
          {games.length === 0 ? (
            <div className="glass-card rounded-3xl p-8 text-center">
              <h2 className="text-2xl font-black">Keine letzten Spiele gefunden</h2>
              <p className="mt-2 text-slate-300">
                Für dieses Team sind noch keine historischen Ergebnisse verfügbar.
              </p>
            </div>
          ) : (
            games.map((g, i) => (
