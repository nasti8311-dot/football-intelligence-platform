      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
          <p className="text-xs font-black uppercase tracking-[0.25em] text-emerald-400">
            Football IQ Match Center
          </p>
          <h1 className="mt-3 text-5xl font-black">Match Center</h1>
          <p className="mt-4 text-sm leading-7 text-neutral-400">
            Nächste 3 Tage mit Odds-Qualität und Datenstatus.
          </p>
        </section>

        <section className="grid gap-4 md:grid-cols-4">
          <Card label="Matches" value={rows.length} />
          <Card label="High/Medium" value={rows.filter((r) => r.quality === "HIGH" || r.quality === "MEDIUM").length} />
          <Card label="Low" value={rows.filter((r) => r.quality === "LOW").length} />
          <Card label="No Odds" value={rows.filter((r) => r.quality === "NO_ODDS").length} />
        </section>
